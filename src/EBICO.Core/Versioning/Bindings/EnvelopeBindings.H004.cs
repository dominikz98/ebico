...
// interfaces (and the ProtocolVersion discriminator) to the generated H004 bindings.
// See EnvelopeBindings.H003.cs for why these live outside Schema/{Hxxx}/.

/// <summary>H004 <c>ebicsRequest</c> envelope (client → server).</summary>
public partial class EbicsRequest : IEbicsRequestEnvelope
{
    /// <inheritdoc/>
    public EbicsVersion ProtocolVersion => EbicsVersion.H004;
}

/// <summary>H004 <c>ebicsUnsecuredRequest</c> envelope (client → server).</summary>
...
/// <summary>H004 <c>ebicsUnsignedRequest</c> envelope (client → server).</summary>
...
/// <summary>H004 <c>ebicsNoPubKeyDigestsRequest</c> envelope (client → server).</summary>
...
/// <summary>H004 <c>ebicsResponse</c> envelope (server → client).</summary>
...
/// <summary>H004 <c>ebicsKeyManagementResponse</c> envelope (server → client).</summary>
